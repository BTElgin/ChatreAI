...
- [ ] Deferred to Phase 13: escalation copy (`ESCALATION_MESSAGE`/`ESCALATION_CTA`) still always says "book a call with a Cadre AI strategist," which reads oddly for an existing customer — routing an existing customer to their account team instead belongs with the lead-capture/human-handoff work, not this tone-only phase

## Phase 13 — Progressive Lead Capture (not started)
- [ ] Extend `ChatState` to accumulate a lead profile across turns (name, business name, business type/category, phone, email) as fields naturally volunteered in conversation are recognized — transparently, never covertly: the bot should ask for contact info outright once a conversation shows real intent, not extract it from unrelated small talk without the user knowing why
- [ ] Skip this flow entirely when `existing_customer` is `true` (Phase 12's flag) — an existing client doesn't need to be profiled as a new lead; route them toward their account team instead
- [ ] Decide the exact trigger for "ask for contact info" (e.g. after N in-scope exchanges, or when a prospect asks a booking/pricing-adjacent question) — needs a concrete threshold, not vague "eventually"
- [ ] Once a lead profile is reasonably complete, deliver it via a webhook to an existing tool (per the user's choice — target tool/URL still needs to be named) rather than a new database, keeping with this project's no-persistent-storage architecture
- [ ] Re-run the 6 core scenarios plus the Phase 10/12 additions to confirm the new profile-collection step doesn't derail unrelated conversations or make the bot pushy
