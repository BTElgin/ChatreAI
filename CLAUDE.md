...
- **Progressive lead capture (Phase 13):** `app/lead.py` + a `lead_capture` graph node (`respond → lead_capture → suggest_followups`). Stateless like everything else here — no session store, so "already asked" / "already delivered" are derived by checking past assistant turns for fixed marker strings (`LEAD_ASK_PROMPT`, `LEAD_DELIVERED_NOTE`, `CUSTOMER_SIGNAL_NOTE`) rather than tracked server-side. For prospects: an explicit `LEAD_ASK_PROMPT` once buying intent (`booking`/`pricing`) shows and there's been a prior exchange, then delivery once name + email/phone are complete (`LEAD_DELIVERED_NOTE` on success, `LEAD_DELIVERY_FALLBACK_NOTE` if `LEAD_WEBHOOK_URL` isn't configured or the POST fails — never claims success it didn't have). `answer()` gets `ANSWER_VOICE_LEAD_PENDING_ADDENDUM` while a lead is pending so it doesn't independently promise a follow-up that `lead_capture` then contradicts — see Automated tests for why that mattered.
