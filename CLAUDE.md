...
## Automated tests
`backend/tests/` — pytest coverage (97 tests) for the LangGraph nodes (`classify`, `answer`, `escalation_check`, `respond`, `lead_capture`, `suggest_followups`, `run_chat`), `app/lead.py`'s pure helpers (`tests/test_lead.py`), and the `/api/chat` / `/api/config` endpoints — including the 6 core scenarios below, the 2 Phase 10 topics (pricing, case studies), the Phase 3 edge cases (multi-part, partial-scope, ambiguous, simulated API failure), model routing (classify/suggest on Haiku, answer on Opus), Phase 12's `existing_customer` flag (defaults false, flips true on self-declaration, selects the right answer voice *and* the right escalation CTA, propagates end-to-end through `run_chat`), and Phase 13's lead capture (every skip condition, the ask trigger, extraction, delivery success/failure/fallback, and a 3-turn end-to-end regression test). The Anthropic client is mocked throughout, so the suite is fast, free, deterministic, and needs no `ANTHROPIC_API_KEY`. It regression-tests the deterministic plumbing — escalation logic, knowledge scoping, request validation, error fallback — not Claude's actual classification judgment; that's what the manual pass below is for.
